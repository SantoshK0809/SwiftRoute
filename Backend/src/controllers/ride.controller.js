          ...
          captainsInRadius.forEach((captain) => {
            if (captain.socketId) {
              io.to(captain.socketId).emit("new-ride", {
                ...
                fare: ride.fare,
                distance:
                  (distanceTime.distance.value / 1000).toFixed(1) + " km",
                duration:
                  Math.round(distanceTime.duration.value / 60) + " mins",
                vehicleType: req.body.vehicleType,
              });
              console.log(
                `Sent ride request to captain ${captain._id} on socket ${captain.socketId}`,
              );
            }
          });
        }
